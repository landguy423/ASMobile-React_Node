...
// ------------------------------------
// TEST Forgot Password actions
// ------------------------------------
test('getProfile', () => {
  expect(actions.forgotPassword.request('test')).toEqual({
    type: constants.FORGOT_PASSWORD_REQUESTED,
    payload: 'test'
  });

  expect(actions.forgotPassword.success()).toEqual({
    type: constants.FORGOT_PASSWORD_SUCCESS
  });

  expect(actions.forgotPassword.failure('test')).toEqual({
...
// ------------------------------------
test('getProfile', () => {
  expect(actions.resetPassword.request('test')).toEqual({
    type: constants.RESET_PASSWORD_REQUESTED,
    payload: 'test'
  });

  expect(actions.resetPassword.success()).toEqual({
    type: constants.RESET_PASSWORD_SUCCESS
  });

  expect(actions.resetPassword.failure('test')).toEqual({
